const { getMembersFromgroupID } = require('./db/admin');
...
const revealGifteesName = async (memberName, groupName, passphrase) => {
  const allMembers = await getMembersFromgroupID({
    TableName,
    groupID: groupName.toLowerCase(),
  });
  const memberFound = allMembers.find(
    (member) => member.secretPassphrase === passphrase
  );

  if (!memberFound) {
    return {
      error:
        'I am unable to find your details, please ensure you have an account setup for santa\'s secret,  to do this go to secretsanta.mineshdesigns.co.uk/enrol. If you have an account already, check you have provided the correct information from the santa\'s secret email.',
    };
  }
  if (memberFound.memberName !== memberName.toLowerCase()) {
    return {
      error:
        'the name provided is incorrect, please check your email for details.',
    };
  }
  if (
    passphrase
    && memberFound
    && memberFound.secretPassphrase !== passphrase
  ) {
    return {
      error: 'Passphrase is incorrect, please check email for details.',
    };
  }
...
const retrieveMemberByPassphrase = async (groupName, passphrase) => {
  ...
  const memberFound = allMembers.find(
    (member) => member.secretPassphrase === passphrase.toLowerCase()
  );
  ...
  if (
    passphrase
    && memberFound
    && memberFound.secretPassphrase !== passphrase.toLowerCase()
  ) {
    return {
      error: 'Passphrase is incorrect, please check email for details.',
    };
    ...
    case 'IntentRequest':
      switch (event.request.intent.name) {
        case 'AMAZON.StopIntent': {
          ...
          context.succeed(
            generateResponse(buildSpeechResponse(stopMessage, true))
          );
          break;
        }
        case 'AMAZON.HelpIntent': {
          ...
          context.succeed(
            generateResponse(buildSpeechResponse(helpMessage, false))
          );
          break;
        }
        case 'RevealSecretSanta': {
          ...
          if (memberName && groupName && passphrase) {
            ...
            if (aloneConfirmation === 'YES') {
              revealGifteesName(memberName, groupName, passphrase).then(
                (response) => {
                  if (response.error) {
                    const errorMessage = `Ok ${memberName} ${response.error}`;
                    context.succeed(
                      generateResponse(buildSpeechResponse(errorMessage, true))
                    );
                  } else {
                    const finalMessage = `Ok ${memberName} your secret santa is ${response}, Happy shopping!`;
                    context.succeed(
                      generateResponse(buildSpeechResponse(finalMessage, true))
                    );
                  }
                }
              );
              ...
              context.succeed(
                generateResponse(buildSpeechResponse(notAloneMessage, true))
              );
            }
          } else {
        ...
        case 'ManageMyWishlist': {
          ...
          if (groupName && passphrase) {
            retrieveMemberByPassphrase(groupName, passphrase).then(
              (response) => {
                if (response.error) {
            ...
            retrieveMemberByPassphrase(groupName, passphrase).then(
              (response) => {
                if (response.error) {
                  ...
                  const secretName = Buffer.from(
                    response.secretSanta,
                    'base64'
                  ).toString('ascii');
                  retrieveGifteesWishlist(groupName, secretName).then(
                    (giftIdeas) => {
                      if (giftIdeas.error) {
                        const errorMessage = `I've gone to feed the reindeers, ${giftIdeas.error}`;
                        context.succeed(
                          generateResponse(
                            buildSpeechResponse(errorMessage, true)
                          )
                        );
                        ...
                        context.succeed(
                          generateResponse(
                            buildSpeechResponse(myGifteesWishlistMessage, true)
                          )
                        );
                      } else {
                        // eslint-disable-next-line max-len
